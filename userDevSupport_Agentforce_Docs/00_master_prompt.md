...
- agentAI_pronpts配下の20個のガイドファイル（00～19）を統合的に実行するための手順書
...
- [ ] 要件変更の扱いと記録方法を確認した（03_change_log.md）
...
7. 変更ログ記録
   └─ 03_change_log.md に記録（方針参照）
      - 一般的な変更: 対象、要約、理由、影響、Branch/PR/Commit
      - 重要な判断分岐: コンテキスト、提案（複数案）、ユーザー決定、実施内容、次のステップ
...
2. ドキュメント作成
   ├─ デプロイ手順書
   ├─ リリースノート
   └─ 変更ログ更新（03_change_log.md）
      - 重要な判断分岐がある場合: コンテキスト、提案、ユーザー決定を明記
...
### ルール7: 整合性維持（20_consistency_check_process）
**Git コミット前に必ず実行**
- コア文書（00-05）更新時: 整合性チェック必須
- 設計文書（design/）更新時: 整合性チェック必須
- ガバナンスルール（agentAI_pronpts/）更新時: 整合性チェック必須
- フェーズ完了・開始時: 整合性チェック必須
- 不整合発見時: HIGH は即座に修正、MEDIUM は修正推奨、LOW は次回対応
- FILE_STRUCTURE_ANALYSIS.md: 構造変更時に更新必須（詳細は 20_consistency_check_process.md と 19_vcs_and_github_workflow.md 参照）
...
- ユーザー要望の切り分け → 施策提案 → 機能要件抽出 → 開発チケット作成の一連フロー
- レコードCRUD操作（検索、参照、作成、更新、削除）とサジェスト機能
- ノンコード開発ガイド（オブジェクト、項目、レイアウト、簡易フロー）の提供

### 除外範囲（スコープ外）
- ユーザー回答としてのApex/LWC等のプロコード提案
- ユーザー回答としての複雑なフロー開発提案（承認プロセス、外部連携、スケジュール実行等）
- システム管理機能（ユーザー管理、権限設定、組織設定等）
- 標準オブジェクトへの構造変更（参照は可能）
- データローダーによる大量データ操作

### 変更許可範囲／禁止操作
- **許可**: カスタムオブジェクトのレコードCRUD、項目追加（ユーザー確認後）、ページレイアウト・レコードタイプ・簡易フロー設定（ユーザー確認後）、Apex/LWCの内部実装と実行（ユーザー確認後）、ガイド資料作成
- **禁止**: ユーザー回答としてのプロコード提案、ユーザー回答としての複雑フロー提案、標準オブジェクトへの構造変更、ユーザー・権限・セキュリティ設定変更、「開発App」対象外オブジェクトへの変更
- **変更履歴**: 03_change_log.md に記録

### データモデル定義（02_data_model_constraints準拠）
- **対象オブジェクト**: Dev_Service__c, Issue__c, TechActionPlan__c, FunctionRequirements__c, Dev_Ticket__c
- **リレーション**: Relation_Support2Issue__c, Relation_Issue2Plan__c, Relation_Plan2Requirements__c, Relation_Requirements2Dev__c
- **標準オブジェクト参照**: Account, Contact, Opportunity, Lead（参照のみ、変更不可）
- **データソース**: 実環境が唯一の正（04_existing_data_model.md で管理）

### 受け入れ基準と必須テスト
- テストカバレッジ基準を満たす（Salesforce標準: 75%以上）
- 正常系、異常系、エッジケーステストの実装
- 主要エラーがない
- ユーザー承認取得済み
- AC-001〜AC-005の受け入れ条件を満たす
...
### 対象環境
- 環境: Developer Edition
- 対話チャネル: Agentforce標準チャット
- 対象範囲: ノンコード開発サポートAgent（要望分析、施策提案、レコードCRUD、開発ガイド）
- データモデル定義: userDevSupport_Agentforce_Docs/04_existing_data_model.md
...
- 前フェーズに戻り、該当箇所を修正
- 修正後、再度フェーズを進める
- ロールバック履歴は 03_change_log.md に記録
...
- **トレーサビリティ**: 全アクションが追跡可能に（03_change_log.md記録必須）
- **品質保証**: データモデル制約準拠、推測禁止、テストカバレッジ75%以上
...
3. 03_change_log.md に記録（重要判断分岐がある場合はコンテキスト・提案・決定を明記）
