...
### 除外範囲（スコープ外）
- Apex/LWC等のプロコード開発全般
- 複雑なフロー開発（承認プロセス、外部連携、スケジュール実行等）
- システム管理機能（ユーザー管理、権限設定、組織設定等）
- 標準オブジェクトへの構造変更（参照は可能）
- データローダーによる大量データ操作

### 変更許可範囲／禁止操作
- **許可**: カスタムオブジェクトのレコードCRUD、項目追加（ユーザー確認後）、ページレイアウト・レコードタイプ・簡易フロー設定（ユーザー確認後）、ガイド資料作成
- **禁止**: プロコード自動生成、標準オブジェクトへの構造変更、ユーザー・権限・セキュリティ設定変更、テストクラス自動生成、「開発App」対象外オブジェクトへの変更
