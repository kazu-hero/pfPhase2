...
## 2026-03-10: 根本的な設計誤りの発覚（プロジェクト再定義が必要）

### コンテキスト
- Phase 3.1-3a スモークテスト継続中に、UI側での実動作を確認
- 重大な設計誤りを発見：「1つのAgent + 5 Topics」構成が根本的に不適切

### 発覚した問題

#### 問題1: 曖昧な要望での誤実行
**事象**:
- ❌ 「メール自動化したい」という曖昧な要望に対し、いきなり自動化設定を開始
- ✅ 「Apexを書いて」という明確な非スコープ要求に対しては正しく拒否

**根本原因**:
- 要望の切り分け・具体化フェーズがスキップされる設計になっている
- Topic 1（Requirement Analysis Support）が入口として機能していない

#### 問題2: 責任範囲の混在
**現在の誤った構成**:
```
UserDevSupportAgent (1 Agent)
 ├─ Topic 1: Requirement Analysis Support（要望切り分け）
 ├─ Topic 2: Record Data Management（レコードCRUD）
 ├─ Topic 3: Non Code Development Guidance（開発ガイド）
 ├─ Topic 4: Custom Object Setup（オブジェクト設定）
 └─ Topic 5: Validation Rule Assistance（検証ルール）
```

**問題点**:
- 1つのAgentで「要望分析 → 施策提案 → 実装支援」のすべてを担当
- Topic機能は「Agent内の専門領域分け」であり、「責任分離」には不十分
- 曖昧な質問が直接実装Topicにルーティングされる

### ユーザーからの指摘

**指摘内容**:
> もしかしてひとつのagentですべてのユーザの要望を実現しようとしていますか？
> 私の想定では、下記のagentはすべて別のagentとして責任範囲を分散すべきです。
> * ユーザー要望・課題の切り分けと分析、類似要望・課題の検索
> * 施策作成と要件定義のサポート
> * SalesforceUI上でのノンコード開発支援（オブジェクト、項目、レイアウト、ページレイアウト等）
>   * 簡易的なフロー開発支援を含む（画面フロー、レコードトリガーフローの基本パターン）

**追加の指摘**:
> 3Agentsで足りるかも不正確な推論です。
> PJ自体を切り直します。

### 根本原因の分析

**Phase 1（要件整理）での誤り**:
1. `01_overview_prompt.md` で「Agentシステム」と表現したが、実際には「1つのAgent」として定義
2. 対象範囲を列挙したが、「複数の独立したAgent」を意味することを明確にしなかった
3. 7つの重大未確定事項への回答が「1 Agent前提」で実施された

**Phase 2（設計）での連鎖**:
- Phase 1の誤った前提を基に「1 Agent + 5 Topics」で設計
- Agent間の責任分離という概念が設計に含まれていない

**Phase 3（実装）での顕在化**:
- 実装してUI上でテストすることで、初めて根本的な誤りが発覚

### 影響範囲

**破棄すべき成果物**:
- Phase 1（要件整理）: 01_overview_prompt.md, 02_solution_definition.md, 05_requirement_definition.mdを全面見直し
- Phase 2（設計）: design/配下のすべてのドキュメント
- Phase 3（実装）: force-app/main/default/配下のAgent metadata

**保持できる資産**:
- agentAI_pronpts/配下のガイドファイル（プロセス定義は正しい）
- Salesforce_DevDocs/配下の技術ドキュメント
- guides/配下のKnowledgeコンテンツ（再利用可能）
- 03_change_log.md（経緯の記録として重要）

### 次のアクション

**即座に実施**:
1. ✅ 現状を03_change_log.mdに記録
2. ⏳ 開発ブランチ（feature/phase3.1-3a-test-improvement）にコミット＆プッシュ
3. ⏳ プロジェクト再定義の進め方を検討

**プロジェクト再定義で決定すべき事項**:
- 何個のAgentで構成するか（3つ？ それ以上？）
- 各Agentの責任範囲と境界
- Agent間の引き継ぎプロトコル
- 共有データモデルの定義
- ユーザーの最初の接点となるAgent

**Branch**: feature/phase3.1-3a-test-improvement
**PR**: -
**Commit**: （本エントリ後にコミット）

**状態**: 🛑 Phase 1からのプロジェクト再定義が必要（根本的な設計誤り発覚）
